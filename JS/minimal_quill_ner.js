...
const BUTTONS = ["PERSON", "PLACE", "ORG", "TITLE", "DATE"];
...
const Up = document
  .getElementById("poly-file")
  .addEventListener("change", e => {
    const filehandle = document.getElementById("poly-file").files[0];
    ...
    CONFIG.reader.addEventListener(
      "load",
      () => {
        readFile();
      },
...
BUTTONS.forEach(item => {
  document
    .getElementById(`${item.toLowerCase()}-button`)
...
const prepareDownload = function(data, filename) {
  const content = JSON.stringify(data).replace(/\[null\]/g, "[[0,0]]");
...
document
  ...
  .addEventListener("click", function() {
    const active_item = parseInt(
      document.getElementById("snippet-counter").textContent
    );
    console.log(active_item);

    source[active_item - 1].delta = document
      .getElementById("editor-container")
      .getElementsByClassName("ql-editor")[0].innerHTML;

    let entities = [];
    quill.getContents().ops.forEach(item => {
      if (item.hasOwnProperty("attributes")) {
        entities.push([
          parseInt(item.attributes.SPAN.start),
          parseInt(item.attributes.SPAN.end),
          item.attributes.SPAN.annotation
        ]);
      }
    });
    let out = {
    ...
    const item = source[active_item - 1];
    Object.keys(source[active_item - 1])
      .filter(key => {
        if (["text", "entities", "annotations"].includes(key)) {
          return false;
        } else {
          return true;
        }
...
//Bulk download
document
  .getElementById("bulk-download-button")
  .addEventListener("click", function() {
    ...
    console.log(active_item);
    source[active_item - 1].annotations = quill.getContents().ops;
    source[active_item - 1].text = quill.getText();
    source[active_item - 1].delta = document
      .getElementById("editor-container")
      .getElementsByClassName("ql-editor")[0].innerHTML;
...
const previous = function() {
  ...
  if (current >= 0) {
    ...
    source[current + 1].annotations = quill.getContents().ops;
    source[current + 1].delta = document
      .getElementById("editor-container")
      .getElementsByClassName("ql-editor")[0].innerHTML;
    console.log(source[current + 1].delta);
    source[current + 1].text = quill.getText();
...
document.getElementById("pre-button").addEventListener("click", function() {
  previous();
});
